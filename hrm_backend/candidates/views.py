...
class CandidateViewSet(viewsets.ModelViewSet):
    ...
    def add_document(self, request, candidate_id=None):
        ...
        # Get existing documents or initialize if empty
        ...
        # Ensure the structure has required and optional categories
        ...
        # Save documents back to the candidate
    ...
    def verify_document(self, request, candidate_id=None):
        candidate = self.get_object()
        doc_type = request.data.get("doc_type")
        category = request.data.get("category", "required")  # Default to "required"
        verifier_id = request.data.get("verified_by")
        
        # Convert documents to dict if it's a string
    ...
    # Same method renamed to match React component expectations
    @action(detail=True, methods=['post'])
