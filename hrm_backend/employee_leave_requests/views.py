...
class Employee_Leave_Request_ListCreateAPIView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return Employee_Leave_Request.objects.filter(status__in=["Pending", "Approved", "Rejected"])
...
class Employee_Leave_Request_ArchiveAPIView(APIView):
    ...
    def post(self, request, leave_id):
        leave = get_object_or_404(Employee_Leave_Request, leave_id = leave_id)
...
class Employee_Leave_Request_ArchiveListAPIView(APIView):
    def post(self, request, leave_id):
        try:
            leave_request = Employee_Leave_Request.objects.get(leave_id=leave_id)
            leave_request.is_archived = True
            leave_request.save()
            return Response({"message": "Leave request archived."})
        except Employee_Leave_Request.DoesNotExist:
            return Response({"detail": "No Employee_Leave_Request matches the given query."}, status=404)
...
class Employee_Leave_Request_HistoryView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if leave_id:
            try:
                leave = Employee_Leave_Request.objects.get(leave_id = leave_id)
...
class Employee_Leave_Request_HistoryView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if leave_id:
            try:
                leave_request = Employee_Leave_Request.objects.get(leave_id = leave_id)
                history = leave_request.history.all()
                serializer = Employee_Leave_Request_HistorySerializer(history, many = True)
